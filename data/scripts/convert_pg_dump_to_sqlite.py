...
PG_DUMP = Path(__file__).parent.parent / "data" / "infra_backup.sql"
OUTPUT = Path(__file__).parent.parent / "data" / "deps_sqlite.sql"
...
if __name__ == "__main__":
    ...
    if not PG_DUMP.exists():
        print(f"ERROR: {pg_dump_path} not found", file=sys.stderr)
        sys.exit(1)
